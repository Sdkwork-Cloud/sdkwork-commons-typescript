...
export function ReadableStreamToAsyncIterable<T>(stream: any): AsyncIterableIterator<T> {
  ...
  export class SdkStream<Item> implements AsyncIterable<Item> {
     ...
     */
    static async streamRequest<Item>(
      url: string,
      options: RequestInit = {},
      client?: any
    ): Promise<SdkStream<Item>> {
      ...
      try {
        ...
        if (contentType && contentType.includes('text/event-stream')) {
          return SdkStream.fromSSEResponse<Item>(response, controller, client);
        } else {
          // Handle as regular JSON stream
          return SdkStream.fromReadableStream<Item>(
